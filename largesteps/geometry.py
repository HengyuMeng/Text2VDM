...
def laplacian_cot(verts, faces, lambda_=None, use_lambdas=False):
    """
    ...
    if use_lambdas:
        idx = torch.arange(
            V, dtype=torch.long, device='cuda')
        sparse_diag_matrix = torch.sparse_coo_tensor(
            torch.stack((idx, idx), dim=0), lambda_, (V, V)).coalesce()
        L = L @ sparse_diag_matrix
    return L
...
def compute_matrix(verts, faces, lambda_, use_lambdas=False, alpha=None, cotan=False):
    """
    ...
    """
    # L   shape : torch.Size([262144, 262144]) 262144 is num of vertices
    # M   shape : torch.Size([262144, 262144])
    # eye shape : torch.Size([262144, 262144])
    if cotan:
        L = laplacian_cot(verts, faces, lambda_, use_lambdas)
    else:
        L = laplacian_uniform(verts, faces)

    idx = torch.arange(verts.shape[0], dtype=torch.long, device='cuda')
    eye = torch.sparse_coo_tensor(torch.stack((idx, idx), dim=0), torch.ones(
        verts.shape[0], dtype=torch.float, device='cuda'), (verts.shape[0], verts.shape[0]))
    if alpha is None:
        # TODO: make lambda_ a learnable parameter （figure out the meaning of L）
        if use_lambdas:
            # idx = torch.arange(
            #     lambda_.shape[0], dtype=torch.long, device='cuda')
            # sparse_diag_matrix = torch.sparse_coo_tensor(
            #     torch.stack((idx, idx), dim=0), lambda_, (lambda_.shape[0], lambda_.shape[0])).coalesce()
            # L_lambdas = L @ sparse_diag_matrix
            M = torch.add(eye, L)
        else:
