...
class StableDiffusion(nn.Module):
    def __init__(self, device, sd_version='2.1', hf_key=None, min=0.02, max=0.98):
        ...
        elif self.sd_version == 'real':
            model_key = "stablediffusionapi/realistic-vision-51"
        else:
            raise ValueError(
    ...
    def get_text_embeds(self, prompt, negative_prompt=[''], batch=1):
        ...
        # for compel
        # prompt here is a list of strings
        conditioning = self.compel.build_conditioning_tensor(prompt[0]) # -> [1, 77, 1024]
        # prompt, negative_prompt: [str]

        # Tokenize text and get embeddings -> [1,77]
        text_input = self.tokenizer(prompt, padding='max_length', max_length=self.tokenizer.model_max_length,
                                    truncation=True, return_tensors='pt') 
            
        with torch.no_grad():
            text_embeddings = self.text_encoder(
                text_input.input_ids.to(self.device))[0] # -> [batch_size, 77, 1024]
           
        B, S = text_embeddings.shape[:2]
        text_embeddings = text_embeddings.repeat(
            1, batch, 1).view(B * batch, S, -1) # -> [1, 77, 1024]
        ...
        uncond_embeddings = uncond_embeddings.repeat(
            1, batch, 1).view(B * batch, S, -1)

        # Cat for final embeddings
        text_embeddings = torch.cat([uncond_embeddings, conditioning])
    ...
    def batch_train_step(self, text_embeddings, pred_rgb,guidance_scale=30,as_latent=False, min_step=0.02, max_step=0.98, phi=0.5, return_t=False):
        ...
        t = torch.randint(min_step, max_step + 1,
                          [1], dtype=torch.long, device=self.device).repeat(B)
        ...
        grad = torch.nan_to_num(grad) # torch.Size([4, 4, 64, 64])
